  ...
  <NavigationMenu>
    <NavigationMenuList>
      <div v-for="item in navigators">
        <div v-if="(item as any).position === NavigationPosition.TOP">
          <NavigationMenuItem v-if="item?.children && item.children.length > 0">
            <NavigationMenuTrigger>{{ $t((item as any)?.group as string) }}</NavigationMenuTrigger>
            <NavigationMenuContent>
              <ul class="grid gap-3 p-6 md:w-[400px] lg:w-[500px] lg:grid-cols-[minmax(0,.75fr)_minmax(0,1fr)]">
                <NavigationItem v-for="navigator in item?.children" :navigator="navigator">
                  <span v-if="(navigator as any)?.description">
                    {{ $t((navigator as any)?.description as string) }}
                  </span>
                </NavigationItem>
              </ul>
            </NavigationMenuContent>
